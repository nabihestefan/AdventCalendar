# To Import:

# import sys
# sys.path.append('../')
# from intCode import *


class Argument:
    def __init__(self, val, addr):
        self.val = val
        self.addr = addr

class Instruction(object):
    code, argNum = 0,0
    def execute(self, intcode, args): pass
    def newPointer(self, intcode):
        return intcode.pointer + self.argNum + 1

class sumInstruction(Instruction):
    code, argNum = 1,3
    def execute(self, intcode, args):
        intcode.set(args[2].addr, args[0].val + args[1].val)
        return self.newPointer(intcode)

class multiplyInstruction(Instruction):
    code, argNum = 2,3
    def execute(self, intcode, args):
        intcode.set(args[2].addr, args[0].val * args[1].val)
        return self.newPointer(intcode)

class inputInstruction(Instruction):
    code, argNum = 3,1
    def execute(self, intcode, args):
        intcode.set(args[0].addr, intcode.getInput())
        return self.newPointer(intcode)

class outputInstruction(Instruction):
    code, argNum = 4,1
    def execute(self, intcode, args):
        intcode.output = args[0].val
        return self.newPointer(intcode)

class jumpIfTrueInstruction(Instruction):
    code, argNum = 5,2
    def execute(self, intcode, args):
        return args[1].val if args[0].val!=0 else self.newPointer(intcode)

class jumpIfFalseInstruction(Instruction):
    code, argNum = 6,2
    def execute(self, intcode, args):
        return args[1].val if args[0].val==0 else self.newPointer(intcode)

class lessThanInstruction(Instruction):
    code, argNum = 7,3
    def execute(self, intcode, args):
        intcode.set(args[2].addr, int(args[0].val < args[1].val))
        return self.newPointer(intcode)

class equalsInstruction(Instruction):
    code, argNum = 8,3
    def execute(self, intcode, args):
        intcode.set(args[2].addr, int(args[0].val == args[1].val))
        return self.newPointer(intcode)

class endInstruction(Instruction):
    code, argument_num = 99, 0
    def execute(self, intcode, arguments):
        intcode.end = True
        return None




class IntCode:
    def __init__(self, program, inputs=[]):
        self.program = program
        self.inputs = inputs[::-1]
        self.output = None
        self.end = False
        self.pointer = 0

    def getInstruction(self, instCode):
        for cls in Instruction.__subclasses__():
            if cls.code == instCode: return cls

    def getArgs(self, argNum):
        modes = str(self.program[self.pointer]).zfill(5)[:3][::-1]
        args = []
        for i in range(argNum):
            val = self.program[self.pointer+i+1]
            if modes[i] == '1':
                args.append(Argument(val,val))
            else:
                args.append(Argument(self.program[val], val))
        return args

    def run(self):
        self.output = None
        while not self.end and self.output == None:
            instruction = self.getInstruction(self.program[self.pointer]%100)
            args = self.getArgs(instruction.argNum)
            self.pointer = instruction().execute(self, args)
        return self.output

    def execute(self):
        lastOut = None
        while not self.end:
            output = self.run()
            if not self.end:
                lastOut = output
        return lastOut

    def set(self, addr, val):
        self.program[addr] = val

    def getInput(self):
        return self.inputs.pop()
