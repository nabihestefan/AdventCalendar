...
for line in open(files[0]):
    if '---' in line:
        assert int(line.split()[2]) == len(scanner_readings)
...
def try_orient_and_align(known_beacons, singleScanReadings):
            ...
            for axis2 in {0, 1, 2} - {axis1}:
                for sign2 in [1, -1]:
                    ...
                    unaligned_beacons = [reorient(reading, *orientation)
                                         for reading in singleScanReadings]
...
def orient_all(known_beacons, known_scanners, unaligned_readings):
    while unaligned_readings:
        progress = False
        for singleScanReadings in list(unaligned_readings):
            beacons, scanner_pos = try_orient_and_align(known_beacons, singleScanReadings)
            if beacons:
                unaligned_readings.remove(singleScanReadings)
...
for x0, y0, z0 in known_scanners:
    for x1, y1, z1 in known_scanners:
        max_dist = max(abs(x1 - x0) + abs(y1 - y0) + abs(z1 - z0), max_dist)
